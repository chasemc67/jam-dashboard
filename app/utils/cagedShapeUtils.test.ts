...
describe('cagedShapeUtils', () => {
  describe('getNotesForStringInShape', () => {
    ...
    it('correctly walks back notes for lower strings in C shape', () => {
      ...
      expect(getNotesForStringInShape(2, 'C')).toEqual([1, 2]); // B: 2nd and 3rd
      expect(getNotesForStringInShape(3, 'C')).toEqual([0, 1]); // G: root and 2nd
      expect(getNotesForStringInShape(4, 'C')).toEqual([4, 0]); // D: 6th and root
      expect(getNotesForStringInShape(5, 'C')).toEqual([3, 4]); // A: 5th and 6th
