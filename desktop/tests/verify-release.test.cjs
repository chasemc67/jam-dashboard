...
test('the update feed must list both architecture zips for the release version', async () => {
    ...
    checkFeed(
      {
        version,
        files: [
          entry('Jam-Dashboard-0.1.42-x64.zip'),
  ...
  assert.deepEqual(
    checkFeed(
      { version, files: [entry('Jam-Dashboard-0.1.42-arm64.zip')] },
      { version },
    ),
    ['latest-mac.yml has no Jam-Dashboard-0.1.42-x64.zip entry'],
  );
